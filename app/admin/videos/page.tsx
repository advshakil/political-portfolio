...
export default function AdminVideosPage() {
    ...
    const [newVideo, setNewVideo] = useState({ url: "", title: "", titleBn: "" })
        ...
        try {
            const res = await fetch("/api/admin/videos", {
                ...
                body: JSON.stringify({
                    ...
                    titleBn: newVideo.titleBn
                }),
            })
            const data = await res.json()
            if (data.success) {
                toast.success("Video added successfully")
                setNewVideo({ url: "", title: "", titleBn: "" })
                fetchVideos()
            } else {
        ...
        <div className="p-4 md:p-8 max-w-6xl mx-auto space-y-8">
            ...
            <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
                ...
                <form onSubmit={handleAddVideo} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 items-end">
                    ...
                    <div className="space-y-2">
                        <Label htmlFor="title" className="text-gray-700">Title (English)</Label>
                        <Input
                            id="title"
                    ...
                    <div className="space-y-2">
                        <Label htmlFor="titleBn" className="text-gray-700">Title (Bengali)</Label>
                        <Input
                            id="titleBn"
                            placeholder="বার্তার শিরোনাম"
                            className="bg-white border-gray-200 text-gray-900 placeholder:text-gray-400"
                            value={newVideo.titleBn}
                            onChange={(e) => setNewVideo({ ...newVideo, titleBn: e.target.value })}
                            required
                        />
                    </div>
                    <Button type="submit" className="bg-[#006A4E] hover:bg-[#005a42] text-white h-11" disabled={submitting}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {loading ? (
                    ...
                    videos.map((video) => (
                        <div key={video._id} className="bg-white rounded-2xl overflow-hidden shadow-sm border border-gray-100 group">
                            <div className="relative aspect-video">
                                ...
                                <div className="absolute inset-0 bg-black/40 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                                    <a
                                        ...
                                        className="p-3 bg-white rounded-full text-[#006A4E] hover:scale-110 transition-transform"
                                    >
